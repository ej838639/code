...
### Inputs ###
change_date = pd.to_datetime('5/16/2018')
cell = 'Minneapolis TAC 43524 Impr Degr'
days_of_change = 1
coverage_category = 'Offender'
results_filename = 'Raw - ' + cell
category_filename = 'KPI Impr Degr Categroy v1'
...
# Only analyze Wednesday (weekday 2)
df = df[df['weekday'] == 2]
...
s_after_incr_category = s_after_incr_sum >= days_of_change # Boolean: True if increase is 5 or more
s_after_decr_category = s_after_decr_sum >= days_of_change # Boolean: True if decrease is 5 or more
...
s_percent_change['UE Power Headroom'] = (s_after_med['UE Power Headroom'] - s_before_med['UE Power Headroom'])/(s_before_med['UE Power Headroom'] + 23)
...
df_after_category = df_after_category[['Cell', 'Change Date', 'Change', 'Improvement Type', 'Assessment', 'Days of Significant Increase', 'Days of Significant Decrease', 'Before Median', 'After Median', '% Change', 'bps change']]
