...
### Inputs ###
change_date = pd.to_datetime('5/29/2018')
cell = 'Chicago TAC 41491 SINR Cells'
days_of_change = 7
results_filename = 'LTE2664_Chicago_TAC41491_SINR'
category_filename = 'KPI Type'
path=r"C:\Users\ejohnso43\Documents\1_2018\Nokia\Features\LTE2664 Load Based PUCCH Region\SINR"
os.chdir(path)
...
# Drop cells that are in after, but missing from before
df_before_cells_overall = df_before_bin.groupby(['CELL']).median()
df_after_cells_overall = df_after_bin.groupby(['CELL']).median()
cells_to_drop = df_after_cells_overall.index.difference(df_before_cells_overall.index)
df_after_cells_overall = df_after_cells_overall.drop(cells_to_drop)

df_before_cells_overall = df_before_cells_overall.drop(['weekday'], axis=1)
df_after_cells_overall = df_after_cells_overall.drop(['weekday'], axis=1)

df_before_cells_overall.to_csv('Median Before - ' + cell + '.csv')
df_after_cells_overall.to_csv('Median After - ' + cell + '.csv')
