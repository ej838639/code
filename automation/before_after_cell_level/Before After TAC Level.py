...
# Median after/after
df_before_bin_wd_med = df_before_bin.groupby(['TAC']).median()
df_after_bin_wd_med = df_after_bin.groupby(['TAC']).median()

# Percentage (%) change
df_bin_percent_change = (df_after_bin_wd_med - df_before_bin_wd_med)/df_before_bin_wd_med

# Lower/upper quartile before
df_before_bin_wd_lower = df_before_bin.groupby(['TAC','weekday']).quantile(q = 0.25)
df_before_bin_wd_upper = df_before_bin.groupby(['TAC','weekday']).quantile(q = 0.75)

# Lower/upper quartile after
df_after_bin_wd_lower = df_after_bin.groupby(['TAC','weekday']).quantile(q = 0.25)
df_after_bin_wd_upper = df_after_bin.groupby(['TAC','weekday']).quantile(q = 0.75)
...
df_after_incr_bin_wd_sum = df_after_bin_wd_incr.groupby('TAC').sum()
df_after_decr_bin_wd_sum = df_after_bin_wd_decr.groupby('TAC').sum()
...
df_assessment_bin.index = df_assessment_bin.index.rename('TAC')
...
df_assessment_bin.to_csv('Assessment TAC - ' + cell + '.csv')
