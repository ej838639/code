...
### Inputs ###
cell = 'Dominance ABA Cluster'
neighbors_filename = 'LTE_Handover - ' + cell
region = 'West'
market = 'Phoenix'
cluster = cell + ' neigh'
path=r"C:\Users\ejohnso43\Documents\1_2018\Dominance\Markets\Phoenix\KPIs\Results"
os.chdir(path)
...
df = df.sort_values(by=['Inter eNB HO Execution Attempts'], ascending = False)
df['Cumulative Handover %'] = df['Inter eNB HO Execution Attempts'].cumsum()/df['Inter eNB HO Execution Attempts'].sum()
# df['Cumulative Handover %'] = pd.Series(["{0:.1f}%".format(val * 100) for val in df['Cumulative Handover %']], index = df.index)
# df = df[df['Cumulative Handover %'] < .95][['Target LNCEL name','Inter eNB HO Execution Attempts','Cumulative Handover %']]

# Create a list of source cells
df_source = df['LNCEL name'].drop_duplicates()
df_source = pd.DataFrame(df_source)

# Add columns needed to create TPIM cluster
df_source = df_source.rename(columns={'LNCEL name': 'CELL_NAME'})

# Filter for only the Top 95% neighbors and only include the target cells
s = df[df['Cumulative Handover %'] < .95]['Target LNCEL name']

# We only included one column, so it create a series, so convert back to a dataframe
df = pd.DataFrame(s)
df = df.rename(columns={'Target LNCEL name': 'CELL_NAME'})

# Add the source cells to the list (otherwise we would only have the neighbor cells)
df = df.append(df_source)


# If we wanted to append the 'cell' string from the input section, then these would have been the commands.
# But we may have more than one source cell, and better to take it from the file
# df = df['Target LNCEL name'].append(pd.DataFrame(pd.Series(cell)))
# df = df.rename(columns={0: 'CELL_NAME'})
...
# Remove duplicates of Cell Name
df = df.drop_duplicates('CELL_NAME')
